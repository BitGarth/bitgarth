use super::helpers::{
    handle_session_expired, prevalidate_ethereum_address_input,
...
use crate::backend::{WalletView, add_ethereum_address, get_settings, get_wallets};
...
use crate::wallets::{AddEthAddressRequest, Network, RawLabel, WalletId};
use crate::{AuthState, BannerState};
use dioxus::prelude::*;

#[component]
pub(crate) fn AddEthereumAddressFlow(
    ...
    // Load wallets and settings for the picker and API key check
    ...
    let save = move |_| {
        ...
        if let Err(err) = prevalidate_ethereum_address_input(&address_raw) {
        ...
        let request = AddEthAddressRequest {
            address: RawEthAddress::new(address_raw),
        ...
        spawn(async move {
            match add_ethereum_address(request).await {
                Ok(response) => {
                    finish_submit(saving);
                ...
                Err(err) if err.is_unauthorized() => {
                    finish_submit(saving);
                    handle_session_expired(auth_state, banner_state, "add ethereum address");
                }
        ...
        div { class: "modal-overlay",
            div { class: "modal",
                div { class: "modal-header",
                    h3 { "Add Ethereum Address" }
                }
                div { class: "modal-body",
                    div { class: "flow-step",
                        p { class: "muted",
                            "Add a watch-only Ethereum address to a new or existing wallet."
                        }

                        if !has_etherscan_api_key {
                            EtherscanApiKeyNotice {}
                        }

                        label { class: "form-label", "Ethereum address" }
                        input {
                            class: "xpub-input",
                            r#type: "text",
                            autocomplete: "off",
                        ...
                        AccountNameField {
                            ...
                            placeholder: "Ethereum Account 1".to_string(),
