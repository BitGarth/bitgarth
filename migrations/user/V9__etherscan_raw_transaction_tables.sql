CREATE TABLE raw_etherscan_normal_transaction_versions (
    ...
    UNIQUE(source_connection_id, tx_hash, payload_bytes)
);
...
CREATE INDEX idx_raw_etherscan_normal_versions_first_observed
ON raw_etherscan_normal_transaction_versions(source_connection_id, first_observed_at DESC);
...
CREATE TABLE raw_etherscan_internal_transaction_versions (
    ...
    CHECK (length(payload_bytes) > 0),
    CHECK (supersedes_raw_version_id IS NULL OR supersedes_raw_version_id != id),
    UNIQUE(source_connection_id, tx_hash, trace_id, payload_bytes)
);
...
CREATE INDEX idx_raw_etherscan_internal_versions_first_observed
ON raw_etherscan_internal_transaction_versions(source_connection_id, first_observed_at DESC);
...
CREATE INDEX idx_raw_etherscan_normal_observations_version_observed
ON raw_etherscan_normal_transaction_observations(raw_etherscan_normal_transaction_version_id, observed_at);
...
CREATE INDEX idx_raw_etherscan_internal_observations_version_observed
ON raw_etherscan_internal_transaction_observations(raw_etherscan_internal_transaction_version_id, observed_at);
